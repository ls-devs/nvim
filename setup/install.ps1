...
function Write-Ok
{ param($msg) Write-Host "[OK]    $msg" -ForegroundColor Green 
}
function Write-Warn
{ param($msg) Write-Host "[WARN]  $msg" -ForegroundColor Yellow 
}
function Write-Err
{ param($msg) Write-Host "[ERROR] $msg" -ForegroundColor Red 
}
function Write-Header
{ param($msg) Write-Host "`n== $msg ==" -ForegroundColor Blue 
}

function Command-Exists
{ param($cmd) return [bool](Get-Command $cmd -ErrorAction SilentlyContinue) 
}
...
} else
{
    ...
    try
    {
        ...
        if (-not (Test-Path $win32yankDir))
        { New-Item -ItemType Directory -Path $win32yankDir | Out-Null 
        }
...
{ Write-Host "  [OK] Go         $(go version | ForEach-Object { $_ -replace '.*go(\S+).*','$1' })" -ForegroundColor Green 
}
if (Command-Exists gh)
{ Write-Host "  [OK] gh CLI     $(gh --version | Select-Object -First 1)" -ForegroundColor Green 
}
if (Command-Exists lazygit)
{ Write-Host "  [OK] lazygit    $(lazygit --version 2>&1 | Select-Object -First 1)" -ForegroundColor Green 
}
if (Command-Exists pnpm)
{ Write-Host "  [OK] pnpm       $(pnpm --version)" -ForegroundColor Green 
}
if (Command-Exists rg)
